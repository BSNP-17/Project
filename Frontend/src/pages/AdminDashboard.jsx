...
const AdminDashboard = () => {
  ...
  const [busForm, setBusForm] = useState({
    ...
    totalSeats: "", availableSeats: "", busType: "AC",
    amenities: "",
  });
  ...
  const fetchBuses = async () => {
    setLoading(true);
    try {
      const res = await adminApi.getAllBuses();
      setBuses(res.data);
  ...
  const resetBusForm = () => {
    setBusForm({ busNumber: "", operator: "", fromCity: "", toCity: "",
      departureTime: "", arrivalTime: "", price: "", totalSeats: "",
      availableSeats: "", busType: "AC", amenities: "" });
  };
          ...
          <div className="dashboard-content">
            <div className="stats-grid">
              ...
              <div className="stat-card green">
                <div className="stat-icon">🚌</div>
                <div className="stat-info">
                  <span className="stat-number">{stats?.totalBuses ?? "—"}</span>
            ...
            <div className="quick-actions">
              ...
              <div className="action-cards">
                <div className="action-card" onClick={() => { setActiveTab("buses"); setShowBusForm(true); }}>
                  <span>➕</span><span>Add New Bus</span>
                </div>
                <div className="action-card" onClick={() => setActiveTab("bookings")}>
                  <span>🎫</span><span>View All Bookings</span>
                </div>
        ...
        {activeTab === "buses" && (
          <div className="buses-content">
            <div className="content-header">
              ...
              <button className="btn-primary" onClick={() => { resetBusForm(); setEditingBus(null); setShowBusForm(true); }}>
                ➕ Add New Bus
              </button>
            </div>
                  ...
                  <form onSubmit={handleBusSubmit} className="bus-form">
                    <div className="form-grid">
                      <div className="form-group">
                        ...
                        <input required placeholder="e.g. KA01AB1234" value={busForm.busNumber}
                          onChange={(e) => setBusForm({ ...busForm, busNumber: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>Operator Name *</label>
                        <input required placeholder="e.g. KSRTC" value={busForm.operator}
                          onChange={(e) => setBusForm({ ...busForm, operator: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>From City *</label>
                        <input required placeholder="e.g. Bengaluru" value={busForm.fromCity}
                          onChange={(e) => setBusForm({ ...busForm, fromCity: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>To City *</label>
                        <input required placeholder="e.g. Mumbai" value={busForm.toCity}
                          onChange={(e) => setBusForm({ ...busForm, toCity: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>Departure Time *</label>
                        <input required type="datetime-local" value={busForm.departureTime}
                          onChange={(e) => setBusForm({ ...busForm, departureTime: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>Arrival Time *</label>
                        <input required type="datetime-local" value={busForm.arrivalTime}
                          onChange={(e) => setBusForm({ ...busForm, arrivalTime: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>Price per Seat (₹) *</label>
                        <input required type="number" placeholder="e.g. 500" value={busForm.price}
                          onChange={(e) => setBusForm({ ...busForm, price: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>Total Seats *</label>
                        <input required type="number" placeholder="e.g. 40" value={busForm.totalSeats}
                          onChange={(e) => setBusForm({ ...busForm, totalSeats: e.target.value })} />
                      </div>
                      <div className="form-group">
                        <label>Available Seats *</label>
                        <input required type="number" placeholder="e.g. 40" value={busForm.availableSeats}
                      ...
                      <div className="form-group full-width">
                        ...
                        <input placeholder="e.g. WiFi, Charging Point, Blanket" value={busForm.amenities}
            ...
            {loading ? <div className="loading">Loading buses...</div> : (
              <div className="table-container">
                <table className="data-table">
                  <thead>
                    <tr>
                      <th>Bus No.</th><th>Operator</th><th>Route</th>
                      <th>Departure</th><th>Type</th><th>Price</th>
                  ...
                  <tbody>
                    {buses.map((bus) => (
                      <tr key={bus.id}>
                        ...
                        <td>₹{bus.price}</td>
                        <td>
                          <span className={bus.availableSeats === 0 ? "seats-full" : "seats-available"}>
                            {bus.availableSeats}/{bus.totalSeats}
                          </span>
                        </td>
                        <td className="action-btns">
                          <button className="btn-edit" onClick={() => handleEditBus(bus)}>✏️ Edit</button>
                          <button className="btn-delete" onClick={() => handleDeleteBus(bus.id)}>🗑️ Delete</button>
                        </td>
                      </tr>
                    ))}
                    {buses.length === 0 && <tr><td colSpan="8" className="empty-row">No buses found</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
            ...
            {loading ? <div className="loading">Loading bookings...</div> : (
              <div className="table-container">
                <table className="data-table">
                  <thead>
                    <tr>
                      <th>Booking ID</th><th>User ID</th><th>Bus ID</th>
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u.id}>
                        ...
                        <td>
                          {u.roles?.map((r) => (
                            <span key={r} className={`role-badge ${r.includes("ADMIN") || r === "admin" ? "role-admin" : "role-user"}`}>
                              {r}
                            </span>
                          ))}
                        </td>
                        <td>
                          {!u.roles?.some(r => r.includes("ADMIN") || r === "admin") && (
